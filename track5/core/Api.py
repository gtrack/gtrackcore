...
def importFile(fileName, genome, trackName):
    ...
    from track5.util.CommonFunctions import createOrigPath, ensurePathExists
    ...
    from track5.preprocess.PreProcessTracksJob import PreProcessAllTracksJob
    PreProcessAllTracksJob(genome, trackName).process()

def _convertTrackName(trackName):
    from track5.util.CommonFunctions import convertTNstrToTNListFormat
...
def _getDirPath(genome=''):
    from track5.util.CommonFunctions import createDirPath, ensurePathExists
...
def listAvailableTracks(genome):
    ...
    from track5.track.hierarchy.ProcTrackNameSource import ProcTrackNameSource
...
def getExtractionOptions(genome, trackName):
    ...
    from track5.extract.TrackExtractor import TrackExtractor
...
def exportFile(outFileName, genome, trackName, fileFormatName, allowOverlaps):
    ...
    from track5.input.userbins.UserBinSource import UserBinSource

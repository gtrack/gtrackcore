...
#from plastid.util.services.exceptions import filterwarnings
            ...
            for field_class in match_order:
                if field_class.matches(field_str):
                    ...
                    if name in self.field_formatters:
                        ...
                        current_type = current_formatter.attr.get("type",
                                                                  current_formatter.__class__.__name__)
...
class SizedAutoSqlField(AutoSqlField):
    ...
    """
    match_str = r"".join([_pattern_bits[_X] for _X in
                          ("start", "type", "size", "name", "optionals", "semi", "comment")])
    match_pattern = re.compile(match_str)
...
class ValuesAutoSqlField(AbstractAutoSqlElement):
    ...
    match_str = r"".join([_pattern_bits[_X] for _X in
                          ("start", "type", "values", "name", "optionals", "semi", "comment")])
    match_pattern = re.compile(match_str)

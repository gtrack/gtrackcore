...
class CythonWigGenomeElementSource(CythonGenomeElementSource):
    _VERSION = '2.0'
    FILE_SUFFIXES = ['wig']
    ...
    #checked directly for speed improvement
    @classmethod
    def _isDeclarationLine(cls, line):
        return line[:9] == 'fixedStep' or line[:12] == 'variableStep'

    # this is faster than startswith
    @classmethod
    def _isFixedStepLine(cls, line):
        return line[:9] == 'fixedStep'
        ...
        if self._fixedStep:
            ...
            if not self._shouldExpandBoundingRegion(chr, start):
                ...
                if self._isStepFunction:
                    returnGE = CythonGenomeElement(genome=self._genome, chr=chr, end=self._start, \
    ...
    def _next(self, line):
        if self._isDeclarationLine(line):
            ...
            return CythonGenomeElement(genome=self._genome, chr=self._chr, start=start, end=end, val=val)
...
class HbWigGenomeElementSource(CythonWigGenomeElementSource):

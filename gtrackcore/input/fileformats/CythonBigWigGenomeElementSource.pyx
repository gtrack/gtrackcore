...
class CythonBigWigGenomeElementSource(CythonGenomeElementSource):
    _VERSION = '1.0'
    ...
    def next(self):
        ...
        if self._fixedStep:
            ...
            if self._isFunction:
                ...
                ge = CythonGenomeElement(genome=self._genome, chr=self._chrName, val=values)
                # print ge

                return ge

        start, end, val = self._parseBigWigFile(header)
        ge = CythonGenomeElement(genome=self._genome, chr=self._chrName, start=start, end=end, val=val)
...
class BigWigGenomeElementSourceForPreproc(CythonBigWigGenomeElementSource):

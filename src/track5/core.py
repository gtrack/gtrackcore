...
class Track5(object):
    ...
    @classmethod
    def is_valid_track5(cls, h5):
        valid = True
        if not cls._get(h5.root._v_attrs, "track5", default=False):
            valid = False
        if cls._get(h5.root._v_attrs, "track5_version", default=None) != cls.TRACK5_VERSION:
            valid = False
        if not cls._get(h5.root._v_attrs, "track5_consistent", default=False):
            valid = False
        if not (
                cls._get(h5.root._v_attrs, "track5_nodes_sealed", default=False)
                and 
                cls._get(h5.root._v_attrs, "track5_links_sealed", default=False)
                ):
            valid = False    
        genome = cls._get(h5.root._v_attrs, "track5_genome", default={})
    ...
    def empty_track(cls, fn, genome, expectedrows=1000000, title="track"):
        ...
        # root attributes
        root = h5.getNode("/")
        root._v_attrs["track5"] = True
        root._v_attrs["track5_version"] = True
        root._v_attrs["track5_genome"] = genome
        root._v_attrs["track5_consistent"] = True
        root._v_attrs["track5_nodes_sealed"] = True
        root._v_attrs["track5_links_sealed"] = True
        ...
        h5.flush()
        h5.close()
        return Track5(fn, mode="r+")
    
    def __init__(self, fn, mode="r"):
        h5 = openFile(fn, mode=mode)
        if self.is_valid_track5(h5):
            self.h5 = h5
    ...
    def _is_sealed(self):
        return self.attrs["track5_nodes_sealed"] and self.attrs["track5_links_sealed"]
    
    def _seal_nodes(self):
        if not self.attrs["track5_nodes_sealed"]:
            self.attrs["track5_nodes_sealed"] = True
        else:
            raise ValueError("Track nodes already sealed")
    
    def _unseal_nodes(self):
        if self.attrs["track5_nodes_sealed"]:
            if self._is_writeable():
                self.attrs["track5_nodes_sealed"] = False
            else:
    ...
    def _seal_links(self):
        if not self.attrs["track5_links_sealed"]:
            self.attrs["track5_links_sealed"] = True
        else:
            raise ValueError("Track links already sealed") 

    def _unseal_links(self):
        if self.attrs["track5_links_sealed"]:
            if self._is_writeable():
                self.attrs["track5_links_sealed"] = False
            else:
                raise ValueError("Track is not writeable")
        else:

...
from gtrackcore_compressed.input.core.GenomeElementSource import BoundingRegionTuple, GenomeElementSource
...
class TrackGenomeElementSource(GenomeElementSource):
    ...
    def _getTrackData(self):
        for br in self._boundingRegions:
            chr = br.chr
            break
        return TrackSource().get_track_data(self._genome, self._trackName, allowOverlaps=self._allowOverlaps)
...
class FullTrackGenomeElementSource(TrackGenomeElementSource):
    def __init__(self, genome, trackName, allowOverlaps=False, *args, **kwArgs):
        ...
        br_handler = BoundingRegionHandler(genome, trackName, allowOverlaps)
        if br_handler.table_exists():
            boundingRegions = list(br_handler.get_all_bounding_regions())
        else:
            boundingRegions = GenomeInfo.getStdChrRegionList(genome)
        GenomeInfo.getStdChrRegionList(genome)
...
class TrackViewListGenomeElementSource(TrackGenomeElementSource):
    ...
    def _getTrackData(self):
        from gtrackcore_compressed.track.core.TrackView import noneFunc

from gtrackcore_compressed.core.DataTypes import getSupportedFileSuffixesForBinning
from gtrackcore_compressed.input.wrappers.GESorter import GESorter
from gtrackcore_compressed.util.CommonFunctions import parseRegSpec, parseShortenedSizeSpec, \
    ...
    def __new__(cls, regSpec, binSpec, genome=None, categoryFilterList=None, strictMatch=True, includeExtraChrs = False): #,fileType):
        if regSpec in ['file', 'track'] + getSupportedFileSuffixesForBinning():
            ...
            if regSpec == 'file':
                ...
                from gtrackcore_compressed.input.adapters.TrackGenomeElementSource import FullTrackGenomeElementSource
            ...
            if categoryFilterList is not None:
                from gtrackcore_compressed.input.wrappers.GECategoryFilter import GECategoryFilter
            ...
            from gtrackcore_compressed.input.userbins.AutoBinner import AutoBinner
            return AutoBinner(parseRegSpec(regSpec, genome, includeExtraChrs=includeExtraChrs), binSize)
    
    @staticmethod
    def _applyEnvelope(geSource):
        from gtrackcore_compressed.input.wrappers.GERegionBoundaryFilter import GERegionBoundaryFilter
        from gtrackcore_compressed.input.wrappers.GEOverlapClusterer import GEOverlapClusterer
...
class ValuesStrippedUserBinSource(UserBinSource):
    @staticmethod
    def _applyEnvelope(geSource):
        from gtrackcore_compressed.input.wrappers.GEMarkRemover import GEMarkRemover

...
from gtrackcore_compressed.metadata.TrackInfo import constructKey, TrackInfo
from gtrackcore_compressed.util.CustomDecorators import timeit
...
class PreProcMetaDataCollector(object):
    ...
    def updateMetaDataForFinalization(self, fileSuffix, prefixList, valDataType, valDim, weightDataType, weightDim, undirectedEdges, \
        ...
        #TODO: Find out where to remove (or add) chr from the _prefixList
        self._checkAndUpdateAttribute('_prefixList', set(prefixList) - set(['chr']), mayBeEmptyString=False, isGeSourceAttr=False, extraErrorMsg='Different formats?')
